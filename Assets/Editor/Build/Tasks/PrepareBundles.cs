...
#pragma warning restore 649
		...
		public ReturnCode Run()
		{
			...
			foreach (string assetGuid in assetGuids)
			{
				...
				string bundleName = m_LayoutManager.GetBundleForAssetPath(assetPath, assetGuid, m_ModificationParameters.Manifest.UniqueName);
				if (bundleName == null)
                {
                    Debug.Log($"Failed to get bundle name for asset {AssetDatabase.GUIDToAssetPath(assetGuid)}");
                    continue;
				}

				if (!m_Tracker.UpdateInfo(assetPath))
                {
                    Debug.Log($"Failed to update info");
				...
				if (assetPath.EndsWith(".unity", StringComparison.OrdinalIgnoreCase))
				{
					...
					Debug.Log($"Found scene: {assetPath}");
		...
		private static void CopyLayout(Dictionary<string, List<GUID>> sourceLayout, Ex.Kingmaker.BundlesLoading.LocationList resultLayout)
		{
