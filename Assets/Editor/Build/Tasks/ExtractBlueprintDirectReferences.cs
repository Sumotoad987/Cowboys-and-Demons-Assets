...
#pragma warning restore 649
		...
		public ReturnCode Run()
		{
			...
			IEnumerable<string> blueprints = Directory.
				GetFiles(m_ModificationParameters.BlueprintsPath, "*.*", SearchOption.AllDirectories).
				Where(path => path.EndsWith("jbp") || path.EndsWith("patch"));
		...
		private Object LoadAsset(string assetId, long? fileId)
		{
            ...
            Debug.Log($"ExtractBlueprintDirectReferences - failed to find asset with guid {assetId} and fileId {fileId?.ToString() ?? "null"}");
            return null;
		}
