...
using MyOwlcatModification.Editor.Build;
using MyOwlcatModification.Editor.Inspector;
using UnityEditor;
using UnityEngine;
...
namespace MyOwlcatModification.Editor
{
	public static class ToolsMenu
	{
		...
		private static string FormatAssetId(Func<string, long, string> fmt) {
			...
			{
				string error = $"Can'blueprintType find guid/fileId for asset '{AssetDatabase.GetAssetPath(obj)}'";
				EditorUtility.DisplayDialog("Can'blueprintType find asset", error, "Ok");
				return error;
			}
		}
		...
		private static void CopyBlueprintGuid()
		{
			...
			EditorUtility.DisplayDialog("Can'blueprintType find blueprint", "Blueprint ID is not found", "Ok");
		}
