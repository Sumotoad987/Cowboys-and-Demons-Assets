...
MaxLayerWeights GetMaxWeights(float2 uv)
{
    ...
    if (_ControlTexturesCount == 0)
    {   
        float4 control0 = tex2D(_Control, uv);
        maxWeights.index0 = 0;
        maxWeights.index1 = 1;
        maxWeights.index2 = 2;
        maxWeights.index3 = 3;
    ...
    else if (_ControlTexturesCount == 1)
    {
...
TerrainLayerData GetTerrainLayerData(int index)
{
    TerrainLayerData result;
    result.masksScale = _TerrainLayerMasksScale[index];
    // нельзя домножать roughness, потому что он блендится некорректно (1 - roughness), художники настраивают маску roughness напрямую в текстуре
    result.masksScale.x = 1;
    result.uvMatrix = _TerrainLayerUvMatrix[index];
    result.uvScale = _TerrainLayerParams[index].x;
    result.diffuseTexIndex = (int) _TerrainLayerParams[index].y;
    result.normalTexIndex = (int) _TerrainLayerParams[index].z;
    result.masksTexIndex = (int) _TerrainLayerParams[index].w;
...
{                             
    ...
    if (_ControlTexturesCount == 0)
    {
        ...
        output.TerrainLayerData0.uvMatrix = fake;
        output.TerrainLayerData1.uvMatrix = fake;
        output.TerrainLayerData2.uvMatrix = fake;
        output.TerrainLayerData3.uvMatrix = fake;
        output.TerrainLayerData0.masksScale = 1;
    }
    else
    {                
        output.TerrainLayerData0 = GetTerrainLayerData(weights.index0);
        output.TerrainLayerData1 = GetTerrainLayerData(weights.index1);
        output.TerrainLayerData2 = GetTerrainLayerData(weights.index2);
        output.TerrainLayerData3 = GetTerrainLayerData(weights.index3);
    }
...
{                            
    ...
    if (_ControlTexturesCount == 0)
    {
        splat.splatXZ0 = tex2D(_Splat0, TRANSFORM_TEX(mapping.uvXZ0, _Splat0));
        splat.splatXZ1 = tex2D(_Splat1, TRANSFORM_TEX(mapping.uvXZ1, _Splat1));
        splat.splatXZ2 = tex2D(_Splat2, TRANSFORM_TEX(mapping.uvXZ2, _Splat2));
    }
    else
    splat = SplatFetch(
...
float4 SplatFetchMasks(TerrainMapping mapping, MaxLayerWeights weights, float4 layersAlpha)
{
    ...
    if (_ControlTexturesCount == 0)
    {
        splat.splatXZ0 = tex2D(_Mask0, TRANSFORM_TEX(mapping.uvXZ0, _Splat0));
        splat.splatXZ1 = tex2D(_Mask1, TRANSFORM_TEX(mapping.uvXZ1, _Splat1));
        splat.splatXZ2 = tex2D(_Mask2, TRANSFORM_TEX(mapping.uvXZ2, _Splat2));
        splat.splatXZ3 = 0;
        //tex2D(_Mask3, TRANSFORM_TEX(mapping.uvXZ3, _Splat3));
        return AlphaBlend(splat.splatXZ0, splat.splatXZ1, splat.splatXZ2, splat.splatXZ3, layersAlpha);
    }
    else
        splat = SplatFetch(
...
{ 
    ...
    if (_ControlTexturesCount == 0)
    {
        splat.splatXZ0 = tex2D(_Normal0, TRANSFORM_TEX(mapping.uvXZ0, _Splat0));
        splat.splatXZ1 = tex2D(_Normal1, TRANSFORM_TEX(mapping.uvXZ1, _Splat1));
        splat.splatXZ2 = tex2D(_Normal2, TRANSFORM_TEX(mapping.uvXZ2, _Splat2));
        splat.splatXZ3 = tex2D(_Normal3, TRANSFORM_TEX(mapping.uvXZ3, _Splat3));                         
    }
    else
    {
        splat = SplatFetch(
