...
GBufferOutput Frag(Varyings input)
{
    ...
    //float4 test = mapping.TerrainLayerData1.diffuseTexIndex;
    //0;                     
    //normalWS = 0;          

    //tex2D(_Mask2, mapping.uvXZ3);
    //masks;
    //SplatFetchDiffuse(mapping, weights, test);
    //tex2D(_Control, input.uv).r * tex2D(_Splat0, _Splat0_ST.xy * input.uv + _Splat0_ST.zw) +
    //tex2D(_Control, input.uv).g * tex2D(_Splat1, _Splat1_ST.xy * input.uv + _Splat1_ST.zw) +
    //tex2D(_Control, input.uv).b * tex2D(_Splat2, _Splat2_ST.xy * input.uv + _Splat2_ST.zw) +
    //tex2D(_Control, input.uv).a * tex2D(_Splat3, _Splat3_ST.xy * input.uv + _Splat3_ST.zw);
    //bakedGI = 0;
    //materialFeatures = 0;
    //shadowMask = 0;  
